...
3. When the agent can read the current `<doc>.md`, prefer
   ...
   operation, priority, quote/prefix/suffix anchors, source hashes, and span
   status without duplicating the document text into context.
...
6. After revising, regenerate the view with `/margin-send <doc>.md` so the author's changes and any replies are reflected; the existing notes ledger is preserved.
7. Optionally append a reply for the user to see in the viewer by editing the note's thread, then regenerate.
